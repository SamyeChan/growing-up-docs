...
![开发密钥（Key）](./static/api-map-tecent.png)
...
<template>
  <!-- 01 需要一个地图的容器 -->
  <div class="map_field" :style="`{width:${VIEWPORT.WIDTH}px;height:${VIEWPORT.HEIGHT - 45}px;`"></div>
</template>
...
export default {
  ...
  methods: {
    ...
    init() {
      // 设置中心点
      var targetLatLng = new qq.maps.LatLng(22.25491, 113.58996)
...
![初始地图](./static/api-map-tecent-init.png)
...
![标记地图](./static/api-map-tecent-sign.png)
...
![多个标记](./static/api-map-tecent-multi-sign.png)
...
![自定义标记](./static/api-map-tecent-selfset-icon.png)

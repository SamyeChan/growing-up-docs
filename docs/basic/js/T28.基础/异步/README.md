...
#### 02 异常 - A 调用 B，无需等待 B 的结果，B 通过状态通知 A 或回调函数来处理；
...
- Promise 对象代表了未来某个将要发生的事情（通常是一个**异步操作**）；
- **好处**：将异步操作以同步操作的流程表达出来，避免了层层嵌套的回调函数；
- **3 种状态**：pending（等待）、resolve（成功）、reject（失败）；
...
![Promise-resolve](./static/promise.png)

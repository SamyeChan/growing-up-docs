...
- 例如：a 标签的 href 功能 - 浏览器给的功能，都是默认事件（默认行为）；
- 阻止 a 标签的默认行为 - 不要浏览器给的功能；
...
![元素跟随鼠标大小变化](./static/1502_tuozhuai.gif)
...
![解决鼠标松开仍可控制 div 问题](./static/1503_tuozhuai.gif)

#### 解决“反向”拖拽不得行问题

- 代表 div 移动距离的 dis 对象应该均为负值；
- div 坐标由初始、当前坐标中的小值决定（div 定位基准：左上角）；
...
![解决“反向”拖拽不得行问题](./static/1504_tuozhuai.gif)
...
![碰撞检测](./static/1602_pengzhuangjiance.gif)
...
![获取元素四条边位置](./static/1601_pengzhuangjiance.png)

- 如上，通过 `el.getBoundingClientRect()`，我们可以拿到元素 4 个方向 `top`、`right`、`bottom`、`left` 的绝对定位值；
...
![基本布局](./static/1701_huakuangxuanze.png)
...
![实现框选功能](./static/1702_huakuangxuanze.gif)
...
![实现框选框中的元素选中](./static/1703_huakuangxuanze.gif)
...
![将选中元素添加至画框中](./static/1704_huakuangxuanze.gif)
